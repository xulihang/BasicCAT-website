...
* 支持调用大模型基于全图给现有区域添加原文、译文、颜色和排序（基于ChatGPTOCR插件通过用OCR给已有区域添加文本操作实现）
* 优化大模型OCR的文字定位，并支持添加译文、颜色等信息
* 用OCR给现有区域添加文本的自定义工作流支持存储额外参数
* 按分镜排序时对分镜外的区域也进行排序
* 修复XYCut排序数组越界的问题
...
* 修复排版阿拉伯语没有启用RTL的问题
* 修复导出的PDF中的文字不支持旋转的问题
* 修复旋转的区域的文字没有居中旋转的问题
* 修复旋转的区域的大小调整
* 修复禁用CJK竖排时报错的问题
* 修复rapidOCR对纯阿拉伯数字的处理
* 修复阿拉伯语PDF生成时对纯阿拉伯语数字的处理

## v6.4.0 (2026/09/03)

* 优化PDF导出选项默认字体的选择
* 导出PDF的自动字体查找功能添加对Windows用户目录的字体文件夹内字体的查找
* 优化导出的竖排文字PDF的文字的间距
* 项目设置没有字体样式的情况导出中日文PDF时，自动检测文字方向
* 自定义工作流支持统计处理时间
* 修改所有默认翻译为百度
* 修复导出的竖排中文翻译版PDF的文字高度不对的问题
* 修复RapidOCR错误反转阿拉伯语数字的问题
* 修复阿拉伯语PDF生成时错误反转阿拉伯语数字的问题

## v6.3.3 (2026/08/28)

* 新增合并重叠区域的自定义工作流
* 新增葡萄牙语（巴西）的语言选项
* 基于目标检测的分镜检测支持在`model.json`定义使用的类别名（`class_names_to_use_for_panel`）
* 修复新建项目不选择语言会报错的问题
* 修复导出含竖排文字的PDF报错的问题
...
* 打开文字去除器会加载项目设置的去除方式
* 如果设置了复杂背景掩膜生成或者图像修复方式，在掩膜编辑器或者文字去除器打开时会使用按区域选择这一方法
* 检测文字方向设置文字样式只有在目标语言支持竖排时执行
* 证书生成器会预先获取IP并使用软件的cert目录作为默认目录
* 修复证书生成器没有正确使用设置的IP的问题

## v6.3.1 (2026/08/13)

修复命令行打开项目没有等待UI调整完毕、OCR语言完成选择就执行后续操作的问题

## v6.3.0 (2026/08/06)

* 安装包默认包含int8量化固定512x512尺寸的Lama Inpaint ONNX模型
* 去除安装包中的MIGAN Inpaint
* 去除安装包中的图像语言分类模型
* 优化CG模板对Lama Inpaint的利用
* 新增对RF-DETR模型的支持
* 新增去除非旋转区域的自定义工作流
* 新增单词大于该长度时可以拆分的项目设置
* 远程模型下载支持校验SHA256
* RapidOCR自动从网络下载模型，切换语言时不显示模型未安装的提示
* 安装包默认包含RapidOCR的泰语和阿拉伯语词典
* 按区域去除文字时，如果区域有重叠，进行合并后再按区域去除，改善重叠导致去字效果差的问题
* 用mangaOCR识别整图时，如果不存在mit48px OCR模型，使用rapid识别长条文字
* 修复掩膜生成和文字去除时检查图像是否复杂的ROI不一致的问题
* 修复预翻译跳过无文本图片的bug

## v6.2.2 (2026/08/01)

* PPDocLayout v3支持mac mlx版本
* 使用全局变量存储分镜检测，避免频繁模型加载
* 去除目标检测的英文翻译的多余空格
* 预翻译跳过无文本图片
...
* 将`Character.UnicodeBlock.GENERAL_PUNCTUATION`（例如…）从汉字字符判断条件中去除
* 更新模板中的最大纵坐标差为2000
* 修复服务器上传错误信息没有传送名字的问题
* 修复服务器可能在上传好翻译的图片前就已经设置运行状态为false的问题

## v6.2.0 (2026/07/26)

* 新增通用服务器模板
* 基于通用语言对模板创建项目时，自动选择合适的OCR
* 新增检测表格分类中单元格的自定义工作流
* 重写所有判断文字区域属于哪个分镜的逻辑
* 使用文字掩膜优化复杂背景图像的判定
* Deepseek默认的模型名改成deepseek-v4-flash
* 分镜检测和翻译工作流支持给每条定义参数
* 优化掩膜生成的反色区域判断
* 服务器支持设置远程服务器密码（ws_secret）
* 服务器支持加载语言专属的项目设置（src.json）
* 服务器自动清理图片功能添加对缩略图的清理
* 修复oneocr的onnxruntime版本冲突问题

## v6.1.0 (2026/07/24)

* 新增基于ZXingCPP的条码扫描功能
* 新增后台运行模式，支持在后台运行服务器，翻译过程不显示窗口
* 批量模式运行工作流的超时时间设置为10分钟
* 服务器支持加载语言专属的自定义工作流模板（custom_workflow_src-tgt.json）
* 修复服务器自定义工作流直接使用之前导出的同名图片的问题
* 修复服务器加载的图片可能无效的问题
...
* 修复通过命令行调用导出PDF时显示完成提示框的问题

## v6.0.1 (2026/07/07)

* 优化竖排引擎对标点避头尾的处理
* 命令行调用服务器的对server参数的判断从包含改为等于
* 提取PDF的图片时，将TIFF转成JPG
* 添加合并重叠区域的气泡检测项目设置
* 合并重叠的气泡区域会检测是否同一类别
* 添加对PPDocLayout v3的支持
* 修复选择气泡检测模型后重新初始化同一模型的实例的问题
* 修复PPDocLayout的内存泄漏问题
* 修复竖排引擎的宽度计算没有考虑描边的问题
* 修复旋转的竖排文字区域描边半径不对的问题
* 修复指定范围导入PDF时进度的范围不对的问题

## v6.0.0 (2026/07/03)

* 横排文字使用精确版字体大小计算
* API偏好设置可以在项目设置中自定义
* 修复旋转的竖排文字区域自动字体大小的bug

## v5.24.0 (2026/06/30)

* 不存储小于0.01的旋转角度
* 新增竖排引擎标点不在行首的项目设置（避头尾）
* 新增拓展译文区域和缩小译文区域的自定义工作流项目
* 排版模式时拓展和缩小区域操作译文位置
* 优化竖排引擎自动文字大小计算的效率
* 修复竖排引擎自动文字大小计算的振荡问题
* 修复竖排引擎在不启用自动换行时自动文字大小对多行文字的处理

## v5.23.0 (2026/06/29)

* 新的OCR插件：oneocr（Windows 11截图OCR）
* OCR先执行图片切分再执行缩放操作
* 修复竖排文字引擎垂直居中无效的问题
* 修复宽度较小的条漫识别的坐标不正确的问题

## v5.22.1 (2026/06/23)

* 通过命令行调用时支持使用已经填写的账号信息进行验证
* 支持使用图片为翻译提供上下文（需要使用视觉大模型）
* 支持使用前几页的文本为翻译提供上下文

## v5.22.0 (2026/06/18)

* 添加最小宽度的OCR项目设置，如果图片小于这一宽度，则放大到这个宽度
* 修复服务器下载失败后没有删除下载失败的文件的问题
* 修复服务器启动失败仍然提示启动成功的问题
...
* 分镜支持保存文本
* 添加使用分镜创建区域的自定义工作流
* 优化竖排文字引擎自动纠偏不同字体大小下效果的统一性
* 修复mit48px OCR可能存在的读取字典的编码问题
...
* 服务器：
   ...
   * 添加定时重启和使用独立进程的偏好设置
   * 保存服务器设置
   * 支持通过命令行直接打开ImageTrans连接服务器
* 掩膜编辑器和文字去除器：
   * 选择方法时会检测插件是否安装或运行
   * 添加按区域选择到处理方法列表，默认不根据是否复杂背景自动选择
* 自定义工作流支持使用多个进程并发执行以优化处理速度
* 修复通过命令行调用模板处理文件不支持单张图片的问题
* 修复macOS上自启动失败的问题
...
* 优化竖排文字引擎对负字间距的支持
* 添加对描边颜色检测的支持
* 富文本编辑器的多用途大小设置控件支持负数
* 修复缩放时翻译文本框发生偏移的问题
* 修复排版模式下使用快速翻译和辅助翻译等方式填充机器翻译时没有实时更新图中结果的问题
...
* 优化图片切换速度
* 多句翻译时跳过没有文本的图
* 修复已翻译图片管理器筛选已经导出翻译的图片的图失败的问题 [#issue1044](https://github.com/xulihang/ImageTrans-docs/issues/1044)

## v5.14.0 (2026/03/06)

* PDF导入：
   * 支持选择页面范围
   * 图像提取模式导入PDF时，如果遇到没有图片的页面，则采用渲染模式
* PDF导出：
   * 支持根据字体名称寻找对应的字体文件
   * 支持选择TTC字体
   * 在mac上的默认字体使用Arial Unicode
   * 如果显示竖排文字，替换括号等标点字符为竖排用字符，替换半角字符为全角字符
   * 优化竖排文字的显示效果
* 增加ChatGPTOCR插件的超时时间
* 添加OllamaOCR插件
* eSCL扫描仪的名称添加eSCL的后缀
* 修正不启用自动换行时竖排文字自动调整大小效果不好的问题
...
* 文档扫描支持使用进纸器（仅eSCL）
* 修复eSCL扫描设置颜色模式无效的问题
* 仿斜体使用仿射变换而不是透视变换以解决和仿粗体同时使用时宽高不对的问题
* 支持通过在根目录放置server-mode文件以避免使用服务器时弹窗报错

## v5.11.0 (2026/01/31)

* 文档扫描支持调用系统扫描接口（WIA、ICA、SANE）
* 文档扫描会保存之前找到的扫描仪列表
* PDF支持高清显示
* 文档模板关闭按区域去除文字和分镜检测检测颜色的设置
* 图像缩放时瓦片加载操作使用防抖
* 导出TXT支持选择导出原文还是原文加译文
* 导入TXT支持导入仅译文
* 修复旋转图像后没有正确更新图像的问题
...
* 优化基于文字区域旋转信息旋转图像对误检测角度的处理
* 优化基于文字区域旋转信息旋转图像对alpha通道的支持
* 优化气泡检测对重叠的和图片外的区域的处理
* 提高应用OCR掩膜的宽容度
* 支持从超星阅读器导入书签
* 修复清空图片报错的问题
...
* 使用双精度保存文字区域的旋转信息
* 添加基于文字区域的旋转信息旋转图像的相关操作
* 添加超分辨率相关操作
* 任意角度旋转图像的调整精度改成0.1
* 支持调用超星阅读器导入PDZ格式的文件
* PDF导出支持更多图像处理参数设置
* 添加分号和冒号到需要转换为中文标点的列表
* 修复编辑图像后仍然使用之前的图像进行OCR等操作的问题
...
* 给超大图片生成缩略图以加速加载
* 掩膜编辑器支持使用鼠标中键切换编辑模式 [#issue1022](https://github.com/xulihang/ImageTrans-docs/issues/1022)
...
* 使用OCR辅助区别图像中的语言是中文还是日文
* 优化以鼠标为中心进行缩放的效果
* 优化缩放的性能
* 修复默认的根据语言配置OCR的模板没有处理拉丁语的问题


## v5.6.0 (2025/12/14)

* 支持使用ONNXRuntime作为目标检测的引擎
* 新增Hot Folder的功能
* 新增检测图片中的语言和检测文本中的语言的功能
* RapidOCR支持自动检测图片中的语言
* 新的工作流：导出PDF、检测图片中的语言以设置OCR、根据文本检测并设置项目源语言
* 支持预设PDF导入导出设置和PSD的导出设置
* 支持通过命令行处理PDF等文件
* 增加依据分镜进行合并的项目设置
* 增加图像修复时融合Alpha通道信息的项目设置
* 增加复杂背景图像的掩膜生成方法和图像修复方法的项目设置
* 修复通过拖拽方式添加项目文件夹的图片失败的问题
* 修复不勾选去除换行时复制文本失败的问题
* 修复服务器会删除自定义工作流配置等问题
...
* 新增复杂背景区域不检测分隔的项目设置
* 新增使用ChatGPT大模型插件进行拼写检查的相关操作
* 新的机器翻译插件：macOS系统自带的翻译
* macOS的LamaInpaint支持调用CoreML模型
* mangaOCR默认使用气泡检测提供定位信息
* RapidOCR新增单词级别的识别
* 图像裁剪支持基于掩膜做透视变换
* API设置支持右键清除设置
* 修复阿拉伯语文字中连续的非阿拉伯词语的合并
* 修复导出阿拉伯语的PDF时连续的非阿拉伯词语没有作为整体反转的问题


## v5.3.0 (2025/11/20)

* 图像编辑增加裁剪和任意角度旋转功能
* 添加法语和越南语到RapidOCR的语言列表
* 添加反转文字的菜单项
* RapidOCR使用v5版阿拉伯语模型
* 搜索与替换添加过滤图片的菜单项
* 已翻译图片管理器支持反选
* 生成PDF时清除文本中的方向控制字符
* 添加去除低置信度区域到自定义工作流
* 修复切换图片后，搜索与替换中的当前图片没有改变的问题
* 修复生成PSD时中间结果目录参数设置错误的问题
* 修复导出的PDF文字层可能超出文档边界的问题
* 修复PDF生成操作不会报错的问题
* 修复生成阿拉伯语的PDF时非阿拉伯语字符被反转的问题

## v5.2.0 (2025/11/15)

* 去除AutoitV3版Photoshop脚本，改成直接调用JavaScript文件
* 在macOS上使用Shell+AppleScript启用Photoshop
* 服务器支持传入工作流、API、项目设置和模板名
* 默认图像修复拓展像素的值从20改成5
* 掩膜如果为全图，去掉边缘几个像素的掩膜内容
* 服务器根据语言选择OCR时，也根据语言选择是否关闭从右到左阅读顺序的选项
* 添加图像修复最大宽度和图像修复滑动窗口重叠比例的设置
* 默认不羽化掩膜
* 项目设置里选中MIGAN、Lama等深度学习图像修复方式时不再取消勾选按文字区域操作
* 使用Krita转换PSD为JPG
* 去除过时的PSD操作
* 生成可编辑PSD支持针对当前图操作
* 修复PS脚本添加去文字图的路径问题
...
* 文字去除器图像可以拖动
* 基于掩膜进行文字去除图片的回填
* 添加回填去文字图时对掩膜进行羽化的项目设置
* 掩膜生成只去除无外部轮廓的轮廓，而不是只保留数量最多的轮廓
* 其它bug修复

## v5.1.0 (2025/11/08)

* 掩膜编辑器新增对选中区域进行掩膜膨胀或侵蚀操作的选项，并更改了默认的操作模式选择方式
* 新增输出格式：光栅化PSD（依赖Krita）
* 重构ORA格式的导出
* 新建的项目，掩膜和去文字图默认保存到`intermediateResults`这个目录
* 更改输出路径为以输出目录为根目录
* 新增SegmentAnything这一掩膜生成器，较适合在复杂背景上的文字
* 新增还原UI状态、生成掩膜图像和生成去文字图像的自定义工作流
* 新增生成当前图片掩膜或去文字图的菜单项
* 支持输出无损WebP
* 优化OCR和气泡检测对长图的内存占用
* 关闭进度框可以停止单张图的OCR和气泡检测操作
* 根据拓展名选择外部方式图像修复的中间格式用JPG还是PNG
* 项目设置里选中MIGAN、Lama等深度学习图像修复方式时会取消勾选按文字区域操作
* 修复文本框重叠部分掩膜生成抵消的问题
* 修复服务器翻译时提前打开了翻译模式的问题
* 修复分镜检测不支持65500像素以上大图的问题
* 修复修改单个区域掩膜时没有更新拓展区域后文字区域的坐标的问题

## v5.0.0 (2025/11/01)

* 掩膜生成模式新增气泡检测
* 用气泡检测和OCR生成掩膜默认只针对全图进行处理
* 掩膜和图像修复方式的设置改用名称存储
* 图像修复只会替换文本框位置的图像
* 新增MIGAN图像修复方式
* 图像修复支持滑动窗口以支持大图或者保留细节
* 优化CG模板的文字去除效果
* 编辑区域的去文字图时会根据项目设置拓展像素
* 文字去除器支持撤回、按区域操作、基于当前图像修复
* 文字去除器和掩膜编辑器的百分比控件支持滚轮控制
* 掩膜的膨胀操作改成针对全图，而不是单个文字区域
* 优化文字去除和掩膜生成的速度
* 更新Lama的插件以支持动态尺寸的输入图像并添加最大尺寸的设置
* 更新Sakura的提示词以避免串行
* ChatGPT使用简化的批量翻译提示词
* 新增是否记住机器翻译结果的偏好设置
* 新增删除当前图片的掩膜和去文字图的菜单项
* 默认图像修复拓展像素的值从5改成20
* 修复图像修复拓展像素后坐标不对的问题
* 其它Bug修复
...
* 排版模式选中文本框时，释放鼠标后会更新字体大小
* 通过工具栏设置字体名时不会同时添加字体大小到本地样式
* 自动文字大小等操作检查本地样式是否有字体大小，而不是检查是否有本地样式
* macOS OCR新增单词级别结果
* 更新ChatGPT插件以支持硅基流动的思考模型
* PDF导出支持使用无文字图或者不添加图片
* PDF导出支持竖排文本、多行文本以及阿拉伯语
* PDF导出的文字层可以设置为显示
* PDF导入支持以单词级别导入文字
* PDF导入选择提取图像时也支持导入文字
* 添加老挝、泰语和高棉语到无空格语言列表
...
* 新增批量调整选中区域或者分镜顺序的功能
* 分镜检测支持指定排序方式和目标检测模型
* 批处理的合并区域操作改用线程执行以避免软件不响应
* 优化插件的加载
* 导出PSD新增计算文字边距的选项
* 新增Sakura翻译插件
* 修复文本仅包含换行时竖排文字引擎渲染报错的问题
* 修复双击设置序号，结果顺序不正确的问题
* 修复文字去除器处理带透明信息图片时没有正确处理alpha值的问题
* 修复WebP图片OCR等操作失败的问题
* 修复另存为后没有更新软件标题和添加最近记录的问题
* 修复根据文字方向确定样式时不存在某个样式会报错的问题
* 其它Bug修复
...
* 新建项目后根据项目语言选择OCR语言
* 新建项目后自动保存项目文件
* 初次打开自定义工作流配置时加载预先定义的工作流
* 使用工具栏修改样式时只保留被修改的样式到本地样式
* 修复Windows的bat和exe的命令行传参问题

## v4.3.0 (2025/09/24)

* 较小图像使用气泡检测时拓宽图片宽度和高度为1024
* 根据合并方向和是否是字符确定是否添加空格与换行
* RapidOCR的字符为单位识别结果保存时会添加`isChar`的属性
* RapidOCR添加阿拉伯语模型
* 气泡检测后默认不拓展区域
* 新增识别长条区域的自定义工作流操作项，去除纯识别OCR识别时可以只用于长条文字的项目设置
* 新增识别可以与有文字区域合并的无文字区域的自定义工作流操作项
* 新增长条区域比例的项目设置
* 更新日漫模板
* 修复气泡检测的坐标可能包含小数的问题
* 修复启用去除换行时只有一个空格的字符会被清除的问题
* 修复自定义工作流本地化带参数的操作项的文本时可能多出一个空格的问题


## v4.2.0 (2025/09/21)

* 竖排文字引擎的自动调整字体大小功能更加准确
* 文字方向检测默认以图片比例为依据
* 新增打开软件目录的菜单项
* 批量翻译操作时气泡检测后也会执行合并区域操作
* 纯识别OCR识别时可以只用于长条文字
* RapidOCR支持返回字符级别的结果
* 默认包含ONNX版mangaOCR
* 基于新的内置OCR优化模板
* 及时释放OpenCV的Mat以优化内存占用
* 批量合并区域改成与UI不相关的操作
* 默认加载ONNXRuntime的动态库
* 添加去除非文字区域（OCR）到自定义工作流
* OCR单个区域时根据存储的位置裁剪而不是控件的位置裁剪
* RapidOCR识别单个区域时默认纠正顺序
* 识别单个区域时调整顺序的选项对mangaOCR等纯识别引擎无效
* 修复百度OCR精确版的文字顺序问题
* 修复自定义工作流带参数项目的本地化问题
* 修复启用排版模式但没有查看翻译时框选文字错误的问题
* 修复自定义工作流启用添加参数时无法添加一般项目的问题
...
* 用OCR给现有区域添加文字可以选择不去除区域外的图像内容
* 恢复水平合并包含文字的区域时拓展边界的行为
* 优化分镜检测速度
* 优化序号显示
* 修复自定义工作流中设置分镜检测方法无效的问题
* 修复选择文本框跳转到图片对应位置和调整顺序时可能存在的index过界问题
...
* 加载WebP时保留透明通道
* 有文字的情况，分隔检测区域以中心为起点
* 使用项目路径作为软件的标题 [#946](https://github.com/xulihang/ImageTrans-docs/issues/946)
* 修复设置的字体样式不存在时无法应用默认样式的问题
* 修复使用服务器版气泡检测默认添加分类名为字体样式的问题
* 修复macOS上按Command键无法进行多选等操作的问题
* 修复PatchMatch无法使用的问题（重新编译了macOS版，并默认使用PNG格式）
